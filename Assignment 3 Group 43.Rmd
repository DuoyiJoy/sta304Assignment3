...
We chose to study how respondents' age, income, sex, education, region of residence, and birthplace influence their fertility intention. For analyzing purposes, we have processed some of the variables to make our analysis concise and easy to follow. For instance, the measurements with missing values on our focused variables were removed. Please see below for a detailed discussion for each of the six variables.

1. Age \newline
Age initially indicates the respondents' age when the survey was conducted, ranging from 15-year-old to 75-year-old. Please see figure1 below for the age distribution of the GSS respondents. For the convenience of comparing the age effect on the fertility rate, we performed a centering technique on the variable of age. Specifically, we subtract the average from each respondent's age and record the difference as a new variable name "ageC." Notice that the age distribution is highly right-skewed, meaning that we have lots of young and middle-aged respondents but a small number of elders. Specifically, the median age is 35 years old and 75% of all respondents are under 43 years old.

```{r age plot, echo=FALSE, fig.cap='Age Spreads of the Respondents',fig.height = 4.5, fig.width = 4.5, fig.align = "center"}
library(ggthemes)
p1 <- gss0 %>% 
  ggplot(aes(x = age)) + 
...
On the other hand, according to figure2 below, the age spread with respect to the binary outcome results (i.e. whether the respondents have fertile intention or not), follows an interesting curvy shape, providing us a hint in terms of model selection, which will be covered later in this report.
...
4. Personal income \newline
Personal income indicates the respondent's income, which is a categorical variable that can take on sixe values:"$\$25,000$ to $\$49,999$", "Less than $\$25,000$", "$\$50,000$ to $\$74,999$", "$\$125,000$ and more", "$\$75,000$ to $\$99,999$", and"$\$100,000$ to $\$124,999$". Below is a summary table that details the number of respondents in each income group. According to figure 6 and 7, 1880 respondents in our data set earn less than $25,000 per year, forming the largest sub-group in terms of income information. On the other hand, there are only 269 respondents in our data set making $125,000 and over every year. To conclude, we have more respondents with low or medium income than wealthy respondents in the data set, which is consistent with the distribution of income in the population.

```{r personal income, eval= TRUE,echo=FALSE}
gss0 %>%
  count(income_respondent) %>%
  knitr::kable(caption = "Figure 6: Personal Income of the Respondents")
```

```{r personal income dist, echo=FALSE, fig.cap='Region of Residency and Income level of the Respondents',fig.align = "center"}
p4 <- gss0 %>%
...
6. Sex \newline
Sex is a categorical variable that can take on two values: male and female. For analyzing purposes, we transformed gender as a dummy variable, a variable can only be either 0 or 1. In our analysis, gender is 1 if the respondent is male or 0 otherwise. According to figure 10 below, we saw that approximately 55% of respondents in our data are female. The gender distribution is relatively balanced among the respondents, as there are 3019 male respondents and 3338 female respondents.
...
ref website: https://www.statisticssolutions.com/assumptions-of-logistic-regression/
...
### keep the calculating eq for later checking purposes
According to coefficient table above, we could obtain the “baseline probability” from our fertile intention estimation model. The baseline probability is the estimated childbearing probability of Alberta female respondents who held a bachelor’s degree and currently reside in Atlantic region, with personal earning ranges from $\$10000$ to $\$12499$. We estimate this probability to be  $p=e^{-5.3}/(1+e^{-5.3} ) =\%$ . We are 95% confident that this probability would be between $p=e^{-5.92}/(1+e^{-5.92} ) \%$ and $p=e^{-4.68}/(1+e^{-4.68} ) \%$.

The age effect on the odds of having children was statistically significant: aging post a negative influence on the odds of having children. As the respondents grow older, they are less likely to plan for childbearing. For one year of age older than the average (i.e. 52-year-old), the respondent's odds of having children would decrease by $1- e^{-0.21}$, vice versa (p<0.005). 

(the log odds of childbearing would decrease by 0.21 as the respondent is one year older than the average) (the respondents’ odds of having children would be $e^{-0.21}\%$

Now, we Shift the attention to gender difference. When the gender of the respondent is male, the odds of him having a fertility intention would increase by e^0.7% in comparison with female respondents with the same economic situation, education level, region of residence and birthplace. The estimated odds are considered to be statistically supported (p<0.005). We are 95% confidence that the amount of increase in odds would fall between   $1-e^{0.57}\%$ and $1-e^{0.89}\%$.

For respondents with various regions of residence areas, the odds of them entering parenthood would differ as well. British Columbia residents have significantly e^0.67-1% higher odds of childbearing than the ones with region in Alberta province. Our regression model also reveals that this increased percentage would be e^0.51% in fertile odds when comparing Prairie residents with Atlantic region residents. 

Noticeably, respondents born in Yukon province or Northwest Territories have odds of childbearing  $e^{-10.19}$  times lower compared to the baseline group with Alberta as birthplace (p<0.05). If the respondents’ birth origin change from Alberta province to New Brunswick, his or her odds of having a fertile plan in life will increase by $e^{0.59}-1\%$. 
...
Last, for the respondents that have different educational backgrounds, the odds of their fertility intention would vary. For university graduates who hold degrees above the bachelors, they tend to have $e^{0.39}$  times higher odds of embracing parental identity, compared to the baseline bachelor group.  While the respondents holding only high school diploma tend to have $1-e^{-0.64}\%$ lower odds of having children (p<0.005).
...
```{r cross validation, include=FALSE, fig.cap="Trade off between Type I and Type II error"}
# =========== Cross Validation ============
...
car::vif(svylogit1)
```
Multicollinearity refers to the situation where the predictor variables in the logistic regression model are highly correlated. To assess this potential issue, variance inflation factors were computed using R, the result indicates two predictors, regions of residency and birth places of the respondents in the model are with relatively high correlation by the rule of thumb (VIF's >5). The solution to this problematic collinearity is through removing one of the concerned variable in the future model conduction.
...
## 5.4 Fertility and Income

Although we have concluded that Income is not a statistically significant predictor of fertility intention, we will still try to reference some of the online articles and give readers a rough idea of why the income variable's coefficient is what it is in our model.

Using respondents who make "$\$100,000$ to $\$124,999$" as a benchmark, the regression shows that wealthy people are the least likely to express fertility intention. In contrast, people with a low-to-medium income are more likely to have fertility intention. Such a finding is counterintuitive initially, as we would predict that poor people should have a lower willingness to enter into parenthood. In fact, raising a child from birth to the age of majority can cost up to $250,000 (National Bank, 2019), which is a considerable amount of money for average families.

On the other hand, wealthy people tend to occupy a higher rank in their organizations, meaning that they have more responsibilities and perform more job functions at the workplace. In other words, wealthy people may end having little time to spend with their family and their beloved ones. Thus, our finding that rich people have a lower fertility intention makes more sense. However, we should be careful not to over-conclude that becoming rich decrease fertility intention. For instance, one can argue that people who initially have a lower willingness to enter parenthood can devote more time to the workplace and thus get promoted and earn more as a result.


## 5.5 Fertility and Gender
...
- Government of Canada, S. (2018, June 11). Report on the Demographic Situation in CanadaFertility: Overview, 2012 to 2016. Retrieved October 14, 2020, from 
...
-	Kate Dartford(2020,March 11).A low birth rate and a rapidly ageing population: Europe’s demographic crisis explained,from 
...
- Smith, J. (2019, October 22). Women less likely to progress at work than male colleagues after childbirth. Retrieved October 17, 2020, from 
https://workplaceinsight.net/women-less-likely-to-progress-at-work-than-their-male-counterparts-following-childbirth/
- Statistics Canada,(n.d.). Retrieved October 16, 2020, from https://www150.statcan.gc.ca/t1/tbl1/en/tv.action?pid=1710013501
...
- Wickham. ggplot2: Elegant Graphics for Data Analysis. Springer-Verlag New York, 2016. https://ggplot2-book.org
